...
For example
suppose the user asks the combined weather of delhi and mumbai
Thought: To find the weather of delhi and mumbai i need to fetch the temperature of both 
one at a time 

Actions:get_weather_data: 25.5941,85.1356 (of delhi)
...
Thought- Now i have the temperature of delhi.. lets go for mumbai
...
Observation:- Delhi has 23 degree temperature and mumbai had 25...so the sum would be 28+21.6=49.6
Answer:I finally have the answer the cobined temperature of delhi and mumbai is 48 degree celsiuis
# Now it's your turn:
...
def iterate(max_iteration=10, userinput: str = ""):
    ...
    while i < max_iteration:
        ...
        if "PAUSE" in result and "Action" in result:
            # Extract the action and arguments using regex
            match = re.search(r"Action:\s*([a-zA-Z_]+):\s*([\d.,\s]+)", result)
            if match:
                function_name = match.group(1)
                arguments = match.group(2).strip().split(",")  # Split arguments
                arguments = [float(arg.strip()) for arg in arguments]  # Convert to float

                # Call the function dynamically
                if function_name in globals():  # Ensure the function exists in the global scope
                    func = globals()[function_name]
                    weather_json = func(*arguments)  # Call the function with arguments
                    relevant_data = extract_relevant_data(weather_json)  # Extract specific data
        ...
        if "Answer" in result:
            break
print("-----------Your personalised weather agent... ask me anything about current weather----------")
